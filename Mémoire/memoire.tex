...
L'encodeur reçoit les séquences de plongement associés aux caractères de chaque cognats et encode les informations qui vont être passées dans le décodeur pour générer une proto-forme plausible. Les deux parties sont chacun des réseaux de neurones à part.\\

Notre première approche a été d'utiliser des modèles de type Transformeur, en espérant profiter de leur performance, pour notre tâche de reconstruction.\\
...
L'exécution de l'expérience nécessitait de développer la quasi-entièreté des algorithmes posés dans le papier de \cite{he2022neural} en Python, un défi rempli d'obstacles, que ce soit du fait du manque au départ de données publiées ou encore de celui du besoin d'appréhender des notions mathématiques nouvelles, en passant par la nouveauté que l'implémentation de réseaux neuronaux a représenté pour nous.
