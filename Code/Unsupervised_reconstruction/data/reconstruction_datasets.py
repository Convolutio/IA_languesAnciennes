...
__TensorCognates = dict[ModernLanguages, Tensor]

# ---------------Maximisation Datasets---------------------

__RawSample = str
__RawCognates = dict[ModernLanguages, str]
...
def __training__collate_fn(batch: list[tuple[__RawSample, __RawCognates, __CachedTargetProbs]]):
    """
    ...
    firstElement = computeInferenceData_Samples(
        wordsToOneHots([t[0] for t in batch]))

    cognates_batch: __TensorCognates = {
        lang: wordsToOneHots([t[1][lang] for t in batch]) for lang in languages}
    ...
    lastElement: __CachedTargetProbs = {lang: {op: pad2d_sequence([t[2][lang][op] for t in batch], 0)[
...
class __TrainingDataset(Dataset[tuple[__RawSample, __RawCognates, __CachedTargetProbs]]):
    def __init__(self, raw_samples: list[__RawSample], raw_cognates: list[__RawCognates], target_probs: list[__CachedTargetProbs]) -> None:
        assert (len(raw_samples) == len(raw_cognates) == len(target_probs)
                ), "The lists to be zipped must have the same length."
    ...
    def __getitem__(self, index: int) -> tuple[__RawSample, __RawCognates, __CachedTargetProbs]:
        return self.training_load[index]


def trainingDataLoader(raw_samples: list[__RawSample],
                       raw_cognates: list[__RawCognates],
...
class __SamplingDataset(IterableDataset[__RawMiniBatch]):
    """
    Returns an iterable of tuple([list of c tuple(b samples tensor, associated cognates)], coords in global batch)
    """

    def __init__(self, proposals: list[Tensor], cognates: list[__TensorCognates],
    ...
    def __iter__(self) -> Generator[__RawMiniBatch, Any, None]:
...
def __sampling__collate_fn(batch: __RawMiniBatch) \
        -> tuple[tuple[InferenceData_Samples, dict[ModernLanguages, InferenceData_Cognates]], __AbsoluteCoords]:
    ...
    collated_cognates: __TensorCognates = {}
    c = len(batch[0][0])
...
def generateSamplingDataset(proposals: list[Tensor], cognates: list[__TensorCognates], batch_shape: __AbsoluteCoords):
    """
    Arguments:
     - batch_shape: (c, b)
    """
