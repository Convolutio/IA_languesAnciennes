...
class ReconstructionModel(nn.Module):
    ...
    def __init__(self, cognatesSet: dict[ModernLanguages, InferenceData], vocab: Vocab, lstm_input_dim: int, lstm_hidden_dim: int):
        super().__init__()
        self.__languages: tuple[ModernLanguages, ...] = tuple(
            cognatesSet.keys())
        ...
        # the size C for the inferences
        self.cachedBatchSize = len(list(cognatesSet.values())[0][1])
    ...
    def renderMiniBatchData(self, samples_: InferenceData, prob_targets_cache: dict[ModernLanguages, ProbCache], miniBatchSize: int):
        ...
        splits: tuple[list[Tensor], list[Tensor]] = (samples_[0].split(
            miniBatchSize, 1), samples_[1].split(miniBatchSize))

        samples_miniBatches: list[InferenceData] = [(splits[0][i], splits[1][i], int(
            torch.max(splits[1][i]).item())) for i in range(len(splits[1]))]
        # = batchSize // MINI_BATCH_SIZE
        miniBatchesNumber = len(samples_miniBatches)

        targets_loads = [torch.empty((0, miniBatchSize, 2*(self.IPA_length+1)), device=device)
                         for _ in range(miniBatchesNumber-1)]  # dim = (*, b, 2*(|Σ|+1))
        ...
        for lang in self.__languages:
            ...
            modern_splits = (model.cachedTargetInputData[0].split(
                miniBatchSize, 1), model.cachedTargetInputData[1].split(miniBatchSize))
            ...
            modernOneHotVectors_miniBatches[lang] = [nextOneHots(
                inpt, self.vocab) for inpt in modern_miniBatches[lang]]
            ...
            for i, (samples_, moderns_, modernOneHots) in enumerate(zip(samples_miniBatches, modern_miniBatches[lang], modernOneHotVectors_miniBatches[lang])):
                ...
                # = MINI_BATCH_SIZE or (batchSize % MINI_BATCH_SIZE)
                thisMiniBatchSize = len(samples_[1])

                mini_targetProbCache = ProbCache(
                    maxSourceLength, maxModernLength, (thisMiniBatchSize, 1))
                mini_targetProbCache.dlt = prob_targets_cache[lang].dlt[:maxSourceLength,
                                                                        :maxModernLength, miniBatchSize*i: miniBatchSize*i + thisMiniBatchSize]
                mini_targetProbCache.end = prob_targets_cache[lang].end[:maxSourceLength,
                                                                        :maxModernLength, miniBatchSize*i: miniBatchSize*i + thisMiniBatchSize]
                mini_targetProbCache.sub = prob_targets_cache[lang].sub[:maxSourceLength,
                                                                        :maxModernLength, miniBatchSize*i: miniBatchSize*i + thisMiniBatchSize]
                mini_targetProbCache.ins = prob_targets_cache[lang].ins[:maxSourceLength,
                                                                        :maxModernLength, miniBatchSize*i: miniBatchSize*i + thisMiniBatchSize]
        ...
        return (
            list(zip(
                samples_miniBatches,
                [dict(zip(modern_miniBatches, t))
                 for t in zip(*modern_miniBatches.values())],
                [dict(zip(modernOneHotVectors_miniBatches, t))
                 for t in zip(*modernOneHotVectors_miniBatches.values())]
            )),
            targets_loads)
    ...
    def renderLogitsForMiniBatch(self, samples_: InferenceData, modern_miniBatches: dict[ModernLanguages, TargetInferenceData], modernOneHotVectors_miniBatches: dict[ModernLanguages, Tensor]):
        # dim = (*, b, 2*(|Σ|+1))
        logits_load = torch.empty(
            (0, len(samples_[1]), 2*(self.IPA_length+1)), device=device)

        samplesInput: SourceInferenceData = self.__computeSourceInferenceData(
            samples_)
        ...
        for lang in self.__languages:
            ...
            logits_load = torch.cat((logits_load, model.get_logits(
                samplesInput, modern_, modernOneHots).flatten(end_dim=1)), dim=0)
        return logits_load
    ...
    def train_models(self, samples_: InferenceData, prob_targets_cache: dict[ModernLanguages, ProbCache], miniBatchSize=30, epochs=5, learning_rate=0.01):
        ...
        # Mini-batching
        numberOfMiniBatches = len(
            samples_[1]) // miniBatchSize + int(len(samples_[1]) % miniBatchSize != 0)
        (miniBatches_data, targets_loads) = self.renderMiniBatchData(
            samples_, prob_targets_cache, miniBatchSize)
        ...
        for epochNumber in range(epochs):
            ...
            for i, (miniBatch_data, targets_load) in enumerate(zip(miniBatches_data, targets_loads)):
                ...
                losses[i] = loss
                print(
                    f'loss: {loss.item():>7f} [minibatch {i+1}/{numberOfMiniBatches}]'+' '*10, end='\r')
    ...
    def forward_dynProg(self, sources_: InferenceData) -> dict[ModernLanguages, Tensor]:
        ...
        with torch.no_grad():
            MINI_BATCH_SIZE = 2
            sources: list[SourceInferenceData] = [self.__computeSourceInferenceData(
                (*s, sources_[2])) for s in zip(sources_[0].split(MINI_BATCH_SIZE, -1), sources_[1].split(MINI_BATCH_SIZE, -1))]
    ...
    def backward_dynProg(self, sources_: InferenceData) -> dict[ModernLanguages, ProbCache]:
        self.eval()
        assert (sources_[0].size()[1:] == (self.cachedBatchSize, 1)
                ), "The expected shape is (C,1), with C the number of cognates pairs passed in the initialisation of the model."
        with torch.no_grad():
