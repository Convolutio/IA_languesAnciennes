...
def compute_mutation_prob(model: EditModel, sources_: InferenceData_SamplesEmbeddings, targets_: InferenceData_Cognates, return_posteriors: bool = False) -> Union[Tensor, ProbCache]:
    ...
    batch_shape = rawSources_lengths.size()
    ...
    f_sub = torch.full((max_source_sequenceLength, max_target_sequenceLength,
                       *batch_shape), fill_value=BIG_NEG, device=device)
    f_ins = torch.full((max_source_sequenceLength, max_target_sequenceLength,
                       *batch_shape), fill_value=BIG_NEG, device=device)
    ...
    assert (
        batch_shape[-1] == 1), 'The backward dynamic program only computes targets probs for one sampled proposal per cognate pair.'

    # the lengths are not according to the boundaries presence
    f_sub_post = torch.full(
        (max_source_sequenceLength, max_target_sequenceLength, *batch_shape), fill_value=BIG_NEG)
    f_ins_post = torch.full(
        (max_source_sequenceLength, max_target_sequenceLength, *batch_shape), fill_value=BIG_NEG)
    ...
    for i in range(max_source_sequenceLength-1, -1, -1):
        for j in range(max_target_sequenceLength-1, -1, -1):
            ...
            if j < max_target_sequenceLength-1:
                ins_post_pr = (f_ins[i, j] + model.ins(i, j) -
                               f_ins[i, j+1]) + f_ins_post[i, j+1]
                f_ins_post[i, j] = torch.logaddexp(
                    f_ins_post[i, j], ins_post_pr)
                posterior_cache.ins[i, j] = ins_post_pr

                sub_post_pr = (f_sub[i, j] + model.sub(i, j) -
                               f_ins[i, j+1]) + f_ins_post[i, j+1]
                f_sub_post[i, j] = torch.logaddexp(
                    f_sub_post[i, j], sub_post_pr)
                posterior_cache.sub[i, j] = sub_post_pr

            if i < max_source_sequenceLength-1:
                dlt_post_pr = (f_sub[i, j] + model.dlt(i, j) -
                               f_sub[i+1, j]) + f_sub_post[i+1, j]
                f_sub_post[i, j] = torch.logaddexp(
                    f_sub_post[i, j], dlt_post_pr)
                posterior_cache.dlt[i, j] = dlt_post_pr

                end_post_pr = (f_ins[i, j] + model.end(i, j) -
                               f_sub[i+1, j]) + f_sub_post[i+1, j]
                f_ins_post[i, j] = torch.logaddexp(
                    f_ins_post[i, j], end_post_pr)
...
def compute_posteriors(model: EditModel, sources_: InferenceData_SamplesEmbeddings, targets_: InferenceData_Cognates) -> ProbCache:
    targetsProbsCache = compute_mutation_prob(
        model, sources_, targets_, return_posteriors=True)
