...
%rajouter peut etre cette partie en annexe
...
\textit{
\begin{itemize}
    \item Démarche non supervisée qui a l'ambition de s'affranchir des bonnes reconstructions associées aux cognats pour entraîner les réseaux. Au contraire il les construit en même temps que d'entraîner les réseaux servant à le faire.
    \item Conséquence, un modèle général plus complexe, entraîné par des EM de type Monte-Carlo. Décrire le rôle des modèles neuronaux dedans et la modélisation de la variation phonétique qui a du être faite pour que ce modèle général ait du sens.
    \item Décrire l'architecture des réseaux
    \item Introduire comment les candidats sont générés, puis expliquer comment l'échantillonnage s'effectue. Discuter sur l'heuristique effectuée à propos des proto-formes possibles pour un set de cognat donné, et émettre une hypothèse sur l'utilité de l'intervention d'un modèle de langue de la proto-langue dans le procédé de sélection.
    \item Expliquer comment les réseaux sont entraînés à partir des échantillons extraits (étape de maximisation)
    \item Terminer cette section en rappelant qu'une expérience sur la reconstruction du latin a été effectuée mais que les éléments appris par les modèles neuronaux d'édition n'ont pas encore reçus d'analyse.
\end{itemize}}

L'ultime modèle étudié dans ce mémoire s'écarte du précédent en proposant de s'affranchir des bonnes reconstructions associées aux cognats pour s'entraîner. Au contraire, il détermine en même temps que d'entraîner les réseaux servant à le faire.
