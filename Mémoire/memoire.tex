...
% Les linguistes historiques étudient les langues anciennes et leur évolution au fil du temps. Au delà d'une simple curiosité, cette étude permet de mieux comprendre les civilisations anciennes à travers leur langue. Leur travail consiste généralement à chercher des correspondances entre ces langues perdues avec nos connaissances actuelles, en comparant des langues pour trouver des liens de parenté et en s'appuyant sur des hypothèses empiriques comme la régularité de l'évolution phonétique des langues. Seulement, même si la chance permet l'accès à des documents écrits, ils restent des langues dont les connaissances sont limitées, rendant ainsi la recherche complexe et chronophage. Depuis quelques décennies, l'intelligence artificielle a su montré ses performances dans le Traitement Automatique des Langues par sa capacité d'analyse et de compréhension des langues contemporaines. Ne pourrait-elle pas s'appliquer pour les langues anciennes ? L'utilisation de cet outil pourrait faciliter et accélérer le travail des linguistes historiques en leur fournissant des hypothèses, par l'analyse de nombreux textes anciens en identifiant des motifs dans les langues perdues.\\   

% Ainsi, une question se pose : le potentiel de l'intelligence artificielle dans la linguistique historique ?\\

% Tout d'abord, nous approfondirons nos connaissances sur la linguistique historique, quels sont les principes sur lesquels elle se repose ? quels sont les problématiques auxquelles elle fait face ? Mais également, nous chercherons à comprendre le fonctionnement d'une intelligence artificielle, et comment elle s'associe avec les langues ?
% Ensuite, nous examinerons l'application de cet outil à deux problèmes de la linguistique historique : la restoration de documents anciens et le déchiffrement de la langues anciennes.
% Enfin, nous étudirons une expérience pour la reconstruction de mots historiques avec de l'intelligence artificielle, dont elle montrera certaines limites.  
...
\begin{equation} \label{eq:combis_m2}
    \begin{split} %TODO: corriger la constante
...
\begin{equation}
    \begin{split}
        v_m = & 4^{m-2} v_2 + \sum_{j=0}^{m-3}\frac{-2}{3}4^{j+2} 4^j + \sum_{j=0}^{m-3}\frac{-1}{3}4^j = 4^{m-2}v_2 - \frac{32}{3} \sum_{j=0}^{m-3}16^j - \frac{1}{3} \sum_{j=0}^{m-3}4^j
    \end{split}
\end{equation}

D'après \ref{eq:combis_m2} et l'expression de $c_{(\lceil \frac{d}{2} \rceil +1)}$, on a $v_2 = 2^{\left(\lceil \frac{d}{2}\rceil + 3\right)}-7$. Pour $m\geq 3$, on obtient donc :
\begin{equation}
    \begin{split}
        v_m = & 4^{m-2}\left(2^{\left(\lceil \frac{d}{2}\rceil + 3\right)}-7\right) - \frac{32}{3}\frac{16^{m-2}-1}{15} - \frac{1}{3}\frac{4^{m-2}-1}{3}\\
        = & 2^{\lceil \frac{d}{2}\rceil+2m- 1} - \frac{32}{45}2^{4m-8} - \frac{64}{9}2^{2m-4} + \frac{37}{45}
    \end{split}
\end{equation}
