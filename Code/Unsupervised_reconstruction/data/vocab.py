...
def make_oneHotTensor(formsVec: Tensor, add_boundaries: bool, formsLengths:NDArray[np.uint8]) -> PackedSequence:
    ...
    t = torch.where(formsVec!=0, formsVec-1, formsVec).numpy() #need numpy for fast indexing in 91st line
    if add_boundaries:
        t = np.concatenate((
    ...
    return pack_padded_sequence(tensor, formsLengths + (2 if add_boundaries else 0), batch_first=False, enforce_sorted=False) #type:ignore
